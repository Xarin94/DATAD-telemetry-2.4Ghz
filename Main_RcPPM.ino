...
#define RCbusrate 100     //delay in milliseconds for the sbus rate
#define RCbuschannels 16  //numers of ppm channels
...
void loop() {
  ...
  if (millis() - RCbusTimer >= RCbusrate && recivedBytes == 0) {
    Message[0] = 255;  //specific for sbus messsage
    Message[1] = RCbusID;
    for (int i = 0; i < RCbuschannels; i++) {
      Message[i + 2] = ppm.latestValidChannelValue(i, 0);
    }
    RCbusTimer = millis();
    recivedBytes=MaxMessageSize;
  }
